...
def run_episode(
    ...
    for step in range(env.cfg.max_episode_steps):
        ...
        elif method in {"initial_dcbf", "refined_dcbf"}:
            if safety_filter is None:
                raise RuntimeError(f"Safety filter is required for method={method}")
...
def main() -> None:
    ...
    for method in methods:
        if method in {"initial_dcbf", "refined_dcbf"}:
            ckpt_path = cfg["initial_checkpoint"] if method == "initial_dcbf" else cfg["refined_checkpoint"]
    ...
    for method in methods:
        if method in {"initial_dcbf", "refined_dcbf"} and method not in filters:
            continue
